...
def evaluate_hybrid_v1_rules(
    ...
    cfg = dict(config or hybrid_v1_config_dict())
    ...
    draft_min_required = float(cfg.get("draft_min_deg", 1.0))
    ...
    wall_min_required = float(cfg.get("wall_mm_min", 1.0))
    wall_max_allowed = float(cfg.get("wall_mm_max", 3.0))
    ...
    if mode_raw is None:
        mode_raw = cfg.get("runner_mode_default", "cold")
