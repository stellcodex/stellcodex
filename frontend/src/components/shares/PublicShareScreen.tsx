...
function ModelPreview({ url }: { url: string }) {
  ...
  return (
    <div className="h-[72vh] overflow-hidden rounded-[var(--radius-xl)] border border-[var(--border-default)] bg-[linear-gradient(180deg,#f7f7f5_0%,#eceee9_100%)]">
...
export function PublicShareScreen({ token }: PublicShareScreenProps) {
  ...
  return (
    <main className="min-h-screen bg-[var(--background-canvas)] px-6 py-8">
      <div className="mx-auto max-w-6xl space-y-6">
        <div className="space-y-2">
          <div className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--foreground-soft)]">Public share</div>
          <h1 className="text-3xl font-semibold tracking-[-0.03em] text-[var(--foreground-strong)]">{share.originalFilename}</h1>
          <div className="text-sm text-[var(--foreground-muted)]">
            Permission: {share.permission} · Download: {share.canDownload ? "allowed" : "blocked"}
          </div>
        </div>
        {share.gltfUrl ? <ModelPreview url={share.gltfUrl} /> : null}
        {!share.gltfUrl && share.originalUrl ? (
          <div className="h-[72vh] overflow-hidden rounded-[var(--radius-xl)] border border-[var(--border-default)] bg-white">
