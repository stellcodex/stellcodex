  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (alive) setError(e instanceof Error ? e.message : "The shared item could not be loaded.");
      }
    })();
    return () => {
      alive = false;
  ...
  return (
    <div className="mx-auto max-w-3xl p-4 sm:p-8">
      <div className="rounded-2xl border border-slate-200 bg-white p-5">
        ...
        {data.expiresAt ? (
          <p className="mt-2 rounded-lg bg-amber-50 px-3 py-2 text-sm text-amber-700">Expires at: {data.expiresAt}</p>
        ) : (
          <p className="mt-2 text-sm text-slate-500">No expiry limit is set.</p>
        )}
        ...
        <div className="mt-4 grid min-h-[260px] place-items-center rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-6 text-center">
          {data.canView ? (
            <div>
              ...
              <div className="mt-1 text-sm text-slate-500">The file preview will appear in this area.</div>
            </div>
          ) : (
            <div className="text-sm text-slate-500">Viewing is not allowed for this link.</div>
          )}
        </div>
