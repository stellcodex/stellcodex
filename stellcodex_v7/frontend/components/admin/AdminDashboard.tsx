  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (alive) setError(e instanceof Error ? e.message : "Admin data could not be loaded.");
      }
    })();
    return () => {
      alive = false;
  ...
  return (
    <div className="grid gap-4">
      <div className="rounded-2xl border border-slate-200 bg-white p-4">
        ...
        <p className="mt-1 text-sm text-slate-600">Jobs, approvals, and system information</p>
      </div>
        ...
        <div className="grid gap-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            ...
            {data.pendingApprovals.length === 0 ? (
              <p className="text-sm text-slate-500">No approvals are waiting.</p>
            ) : (
