...
class Quote(Base):
    ...
    # Geometry snapshot
    geometry_summary: Mapped[Optional[dict]] = mapped_column(JSON)
    # Cost breakdown for qty=1
    breakdown_json: Mapped[Optional[dict]] = mapped_column(JSON)
    # Quantity price breaks
    qty_breaks_json: Mapped[Optional[list]] = mapped_column(JSON)
...
class ProductionOrder(Base):
    ...
    customer_po: Mapped[Optional[str]] = mapped_column(Text)  # customer's PO reference
