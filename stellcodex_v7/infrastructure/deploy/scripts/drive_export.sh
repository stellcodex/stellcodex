...
V10_REPORT_PATH="${ROOT_DIR}/_jobs/reports/stellcodex_v10_engineering_report.json"
V10_REPORT_SUMMARY="${RUN_DIR}/v10_report_upload.json"
...
if [[ -f "${V10_REPORT_PATH}" ]]; then
  "${UPLOAD_HELPER}" "${V10_REPORT_PATH}" "${REMOTE_BASE}/reports" "$(basename "${V10_REPORT_PATH}")" > "${V10_REPORT_SUMMARY}"
fi
...
python3 - <<'PY' "${MANIFEST_JSON}" "${REMOTE_BASE}" "${DB_DUMP_SUMMARY}" "${DB_SHA_SUMMARY}" "${STORAGE_SUMMARY}" "${V10_REPORT_SUMMARY}" "${DB_DUMP_PATH}" "${DB_SHA_PATH}" "${STORAGE_DIR}" "${V10_REPORT_PATH}"
import json
import sys
...
v10_report_summary = load_summary(sys.argv[6])
db_dump_path = sys.argv[7]
db_sha_path = sys.argv[8]
storage_dir = sys.argv[9]
v10_report_path = sys.argv[10]
...
payload = {
    ...
    "engineering_report": {
        "local_path": v10_report_path if Path(v10_report_path).exists() else None,
        "upload": v10_report_summary,
    },
}
