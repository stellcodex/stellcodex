...
const checks: Array<[string, () => void | Promise<void>]> = [
  ...
  const middlewareSource = fs.readFileSync(path.join(frontendRoot, "middleware.ts"), "utf8");
  ...
  assert.equal(middlewareSource.includes('new URL("/sign-in"'), true);
  assert.equal(middlewareSource.includes('"/settings"'), true);
  }],
