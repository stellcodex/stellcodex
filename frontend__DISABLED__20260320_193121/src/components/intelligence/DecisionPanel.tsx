...
export function DecisionPanel({ decision }: DecisionPanelProps) {
  return (
    <Panel description="Decision fields are shown from deterministic backend output only." title="Decision">
      {!decision ? (
        ...
        <div className="space-y-4 text-sm">
          ...
          <div className="space-y-2">
            <div className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--foreground-soft)]">Rule explanations</div>
            {decision.explanations.map((item) => (
              <div key={item.ruleId} className="rounded-[var(--radius-md)] border border-[var(--border-muted)] px-3 py-3">
                <div className="text-sm font-medium">{item.ruleId}</div>
                <div className="mt-1 text-xs text-[var(--foreground-muted)]">{item.reference}</div>
